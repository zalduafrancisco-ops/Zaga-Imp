# -*- coding: utf-8 -*-
"""Propuesta Cristóbal — Socio Logístico Integral (versión limpia y final).
Sin desglose costos, sin IVA importación, sin Form F destacado, sin pasos a seguir.
"""
import os
...
OUT = r"C:\Users\zaldu\OneDrive\Documentos\2. ZAGA\02. Importaciones\Propuesta_Cristobal_Socio_Logistico_2026-05-13.pdf"
doc = SimpleDocTemplate(OUT, pagesize=A4,
                        rightMargin=1.8*cm, leftMargin=1.8*cm,
                        topMargin=1.6*cm, bottomMargin=1.6*cm,
                        title="Propuesta ZAGA — Socio Logístico Integral · Cristóbal")
story = []
...
story.append(Paragraph(
    "Estimado Cristóbal, mi propuesta no es una operación aislada. Le ofrezco ser su "
    "<b>socio logístico integral en la ruta China-Chile</b>: compra, flete, aduana, despacho "
    "y entrega final — todo bajo una sola gestión profesional con apoyo en cada paso de la cadena.",
    S_BODY
))
story.append(Paragraph(
...
beneficios_aduana = [
    ...
    [td("Consolidación de modelos", bold=True),
     td("Múltiples productos en un solo despacho aduanero, evitando duplicación de costos fijos.")],
...
# ═══════════════════════════════════════════
story.append(PageBreak())
story.append(Paragraph("Propuesta de precios — 4 cotizaciones consolidadas", S_H1))
story.append(Paragraph(
    "Los siguientes son precios <b>integrales todo incluido</b> por unidad. Como cliente "
    "preferente inaugural, estos precios consideran el <b>margen más bajo posible</b> que "
    "puedo ofrecer manteniendo el estándar profesional del servicio.",
    S_BODY
))
...
precios = [
    ...
    [td("COT-096", bold=True), td("Pulsera de cadena con dijes"),
     td("2.000", right=True), td("$1.790", bold=True, right=True), td("$3.580.000", right=True)],
    [td("COT-097", bold=True), td("Pulsera de trébol 4 hojas"),
     td("2.000", right=True), td("$1.160", bold=True, right=True), td("$2.320.000", right=True)],
    [td("COT-098", bold=True), td("Pulsera cuentas redondas"),
     td("2.000", right=True), td("$790", bold=True, right=True), td("$1.580.000", right=True)],
    [td("COT-099", bold=True), td("Collar alas ángel personalizado"),
     td("2.000", right=True), td("$2.680", bold=True, right=True), td("$5.360.000", right=True)],
    [td("", bold=True), td("", bold=True), td("<b>8.000 und</b>", bold=True, right=True),
     td("<b>TOTAL c/IVA</b>", bold=True, right=True), td("<b>$12.840.000</b>", bold=True, right=True)],
]
...
incluye = [
    ...
    "Flete aéreo CIP Santiago desde Yiwu hasta aeropuerto SCL.",
...
story.append(Paragraph(
    "Estas 4 cotizaciones son aéreas debido a la urgencia. Sin embargo, para volúmenes "
...
story.append(Paragraph(
    "Mi proveedora china tiene fortaleza especial en marítimo: <b>cuenta con contenedores "
    "saliendo regularmente y optimiza la consolidación de carga</b>, lo que permite un mejor "
    "aprovechamiento del espacio y un costo de flete más eficiente comparado con aéreo.",
    S_BODY
))

story.append(Paragraph(
    "<b>Plazos:</b> el tránsito marítimo desde Yiwu hasta Santiago es de aproximadamente "
...
beneficios = [
    ...
    [td("Margen preferente", bold=True),
     td("Los precios más bajos que ZAGA puede ofrecer manteniendo el estándar profesional del servicio. Se mantiene este beneficio en todas las operaciones futuras.")],
...
def add_footer(canvas, doc):
    ...
    canvas.drawCentredString(A4[0]/2, 0.8*cm,
        f"ZAGA Logística  ·  Propuesta Socio Logístico — Cristóbal  ·  Página {doc.page}")
    canvas.restoreState()
