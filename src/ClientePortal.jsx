...
const EST_COLOR = {
  ...
  rechazada_cliente:"#ef4444", anulada:"#6b7280", aceptada:"#22c55e",
...
const fmt = function(n){ return (!n&&n!==0)?"/bin/sh":Number(n).toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0}) }
    ...
    <div style={{background:"#f1f5f9",minHeight:"100vh",fontFamily:"'Inter','Segoe UI',sans-serif",color:"#0f172a"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .zfbtn{background:#fff;border:1px solid #e2e8f0;cursor:pointer;padding:8px 16px;font-size:13px;font-weight:500;border-radius:20px;font-family:inherit;color:#64748b;transition:all .2s}
          ...
          <div>
            ...
            <div style={{marginBottom:20}} className="zfade">
              ...
              <div style={{fontSize:13,color:"#64748b",marginTop:4}}>
                {enProceso.length>0?"Tienes "+enProceso.length+" importacion"+(enProceso.length>1?"es":"")+" activa"+(enProceso.length>1?"s":"")+".":"Todo al dia con tus importaciones."}
              </div>
            </div>
                  ...
                  {pendientesConf.map(function(c){
                    return (
                      <div key={c.id} onClick={function(){ abrirCot(c.id) }}
                        style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",border:"1px solid #fde68a",borderRadius:10,padding:"10px 14px",cursor:"pointer",transition:"border-color .2s"}}
            ...
            {proxLlegada&&(function(){
              ...
              var dColor = dias<=15?"#ea580c":dias<=30?"#d97706":"#2563eb"
              var dBg = dias<=15?"#fff7ed":dias<=30?"#fffbeb":"#eff6ff"
              return (
            ...
            {/* STATS */}
            <div className="stats-row zfade" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
              {[
                {label:"Total",value:todas.length,icon:"📦",color:"#2563eb",bg:"#eff6ff"},
                {label:"En proceso",value:enProceso.length,icon:"⚙️",color:"#ea580c",bg:"#fff7ed"},
                {label:"En camino",value:enCamino.length,icon:"🚢",color:"#7c3aed",bg:"#f5f3ff"},
                {label:"Completadas",value:completadas.length,icon:"✅",color:"#16a34a",bg:"#f0fdf4"},
              ].map(function(s){
                return (
                  <div key={s.label} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden"}}>
                    <div style={{position:"absolute",top:10,right:12,fontSize:20,opacity:.15}}>{s.icon}</div>
                    <div style={{fontSize:10,color:s.color,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6}}>{s.label}</div>
            ...
            {/* TOTAL INVERTIDO */}
            <div className="zfade" style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:14,padding:"16px 20px",marginBottom:20,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
              <div>
            ...
            {/* FILTROS + BUSQUEDA */}
            <div className="zfade" style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
            ...
            {filtradas.length===0?(
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {filtradas.map(function(c,idx){
                  ...
                  return (
                    <div key={c.id} id={"cot-"+c.id} className="zcard zfade" style={{animationDelay:(idx*0.04)+"s"}}>
                      ...
                      <div className="cot-header" onClick={function(){ setOpenId(isOpen?null:c.id) }}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          {/* Producto */}
                          <div style={{fontSize:15,fontWeight:700,color:"#0f172a",marginBottom:6,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.producto}</div>
                          {/* Chips */}
                        ...
                        <div className="cot-meta" style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
                          ...
                          {tot>0&&<div style={{fontSize:14,fontWeight:800,color:"#040c18"}}>{fmt(tot)}</div>}
