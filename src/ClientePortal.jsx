...
const getImagenes = function(url){ return url ? url.split('|||').filter(Boolean) : [] }
// Proxy de imágenes para dominios con hotlink protection (Alibaba, Taobao, 1688, etc.)
const proxyImg = function(url){
...
export default function ClientePortal({ supabase, perfil, onLogout }) {
  ...
  var [imgIdx, setImgIdx] = useState({})               // {cotId: índice imagen activa carousel}
                                ...
                                return (
                                  <div key={x.cot.id} style={{background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,padding:"12px 14px"}}>
                                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,flexWrap:"wrap"}}>
                                      {img && <img src={proxyImg(img)} alt={x.cot.producto} referrerPolicy="no-referrer" onError={function(e){e.target.style.display='none'}} style={{width:48,height:48,objectFit:"cover",borderRadius:8,border:"1px solid #e2e8f0",flexShrink:0}}/>}
              ...
              <div style={{marginBottom:14}}>
                ...
                <input type="url" value={nuevaForm.url_referencia}
                  ...
                  onPaste={function(e){
                    ...
                    if(clean !== raw){
                      e.preventDefault()
                      setNuevaForm(function(p){ return Object.assign({},p,{url_referencia: clean}) })
                      showToast("✓ URL extraida del texto pegado","ok")
                    }
                  }}
                  placeholder="https://alibaba.com/product/..."
                  style={{width:"100%",padding:"10px 12px",border:"1px solid #cbd5e1",borderRadius:8,fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}/>
                <div style={{fontSize:10,color:"#94a3b8",marginTop:4,fontStyle:"italic"}}>
                  💡 Tip: si copias el texto completo desde Alibaba, se extrae solo la URL automáticamente.
                </div>
              </div>
              ...
              <div style={{marginBottom:14}}>
                ...
                {nuevaForm.url_imagen && (
                  <div style={{marginTop:8,display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <span style={{fontSize:10,color:"#64748b",fontStyle:"italic"}}>Vista previa</span>
                  </div>
                )}
              </div>
