...
export default function App({ supabase, usuario, onLogout }){
  ...
  useEffect(()=>{
    (async()=>{ try{
      ...
      if(data&&!error){ const list=data.map(r=>typeof r.datos==='string'?JSON.parse(r.datos):r.datos); cotizacionesRef.current=list; setCotizaciones(list); }
    }catch(_){} })();
    const canal=supabase.channel('cotizaciones_admin')
      .on('postgres_changes',{event:'*',schema:'public',table:'cotizaciones'},async()=>{
        const { data }=await supabase.from('cotizaciones').select('datos').order('created_at',{ascending:false});
        if(data){ const list=data.map(r=>typeof r.datos==='string'?JSON.parse(r.datos):r.datos); cotizacionesRef.current=list; setCotizaciones(list); }
      }).subscribe();
  ...
  const persist=useCallback(async (newList)=>{
    const oldList=cotizacionesRef.current;
    cotizacionesRef.current=newList; setCotizaciones(newList);
    try{
      const oldIds=new Set(oldList.map(c=>c.id)); const newIds=new Set(newList.map(c=>c.id));
      const deletedIds=[...oldIds].filter(id=>!newIds.has(id));
      if(deletedIds.length>0) await supabase.from('cotizaciones').delete().in('id',deletedIds);
      if(newList.length>0){
        const rows=newList.map(c=>({id:c.id,nro:c.nro||'',cliente:c.cliente||'',gestor:c.gestor||'francisco',estado:c.estado||'solicitud',tipo:c.tipo||'cliente',datos:c}));
        await supabase.from('cotizaciones').upsert(rows);
      }
      ...
      <div style={{background:"linear-gradient(180deg,#0a1525 0%,#080f1c 100%)",borderBottom:"1px solid #1a2d45",padding:"0 24px",boxShadow:"0 2px 20px #000a"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:36,height:36,background:"linear-gradient(135deg,#c9a055,#8a6a30)",borderRadius:9,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,boxShadow:"0 2px 12px #c9a05540"}}>🌏</div>
            <div>
              <div style={{fontWeight:800,fontSize:15,letterSpacing:1.5,color:"#dce8f0",fontFamily:"'DM Serif Display',serif"}}>ZAGA Import</div>
              <div style={{fontSize:9,color:"#c9a05580",letterSpacing:2,textTransform:"uppercase"}}>Gestión de Importaciones</div>
            </div>
          </div>
          <div style={{display:"flex",gap:8,fontSize:12,alignItems:"center"}}>
            ...
            <div style={{width:1,height:20,background:"#1a2d45",margin:"0 4px"}}/>
            <span style={{fontSize:11,color:"#4a5a6a"}}>{usuario?.nombre||"Admin"}</span>
            <button onClick={onLogout} style={{background:"transparent",color:"#c0392b80",border:"1px solid #c0392b33",borderRadius:8,padding:"5px 12px",fontSize:11,cursor:"pointer",fontWeight:600}}>Salir</button>
          </div>
        </div>
